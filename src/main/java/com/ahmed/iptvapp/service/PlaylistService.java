...
public class PlaylistService {
    ...
    /**
     * Create a new playlist from uploaded file
     */
    @Transactional
    public PlaylistDto createFromFile(MultipartFile file, String userId) {
        try {
            Playlist playlist = m3uParserService.parseFromFile(file, userId);
            PlaylistDto playlistDto = savePlaylistWithContent(playlist);
            ...
            log.error("Error creating playlist from file", e);
            throw new RuntimeException("Failed to parse uploaded file: " + e.getMessage());
        }
    }
    ...
    /**
     * Refresh playlist content from its URL
     */
    @Transactional
